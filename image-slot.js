...
(() => {
  ...
  const stylesheet =
    ...
    ':host([data-panning]) .frame img,:host([data-reframing]) .frame img{touch-action:none}' +
  ...
  class ImageSlot extends HTMLElement {
    ...
    _render() {
      ...
      // An unresolved template hole ("{{ foo }}") is not a URL — treat it as
      // absent so the slot never fetches it while values are still streaming.
